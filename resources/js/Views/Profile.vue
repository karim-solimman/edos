<template>
    <v-container>
        ...
        <v-row v-if="!loading">
            <v-col cols="12" lg="5" md="4">
                ...
                <v-chip
                    ...
                    v-for="role in $store.getters.getUser.roles"
                    :key="role.id"
                    ><v-icon small left>mdi-account</v-icon
                    >{{ role.name }}</v-chip
                >
                <v-chip small outlined class="text-overline"
            ...
            <v-col class="d-flex my-auto">
                <h1 class="text-h1 font-weight-light">
                    {{ invs.length
                    }}<span v-if="settings[0].value" class="text-body-1">
                        of ({{ user.invs_limit }})</span
                    >
                    <span class="text-overline">invs</span>
                </h1>
            </v-col>
            <v-col
                v-if="!loading && invs && invs.length > 0"
        ...
        <v-row
            v-if="!loading && invs.length === 0 && roles && roles.length > 0"
        >
            <v-col>
                <v-alert
                    border="top"
            ...
            <v-col lg="4" md="4" cols="12" v-for="inv in invs" :key="inv.id">
                <v-hover v-slot="{ hover }">
                    ...
                    >
                        ...
                        <v-card-subtitle
                            >Time:
                            {{ inv.date_time | TimeFormat }}</v-card-subtitle
                        >
                        <v-card-text>
                            <v-chip-group active-class="primary--text" column>
                                <v-chip
                                    ...
                                    small
                                    :color="
                                        inv.users_count < inv.room.users_limit
                                            ? 'green darken-2'
                                            : 'red darken-2'
                                    "
                                >
        ...
        >
            <v-col lg="4" md="4" cols="6" v-for="inv in invs" :key="inv.id">
                <v-card color="grey lighten-5" hover>
                    ...
                    <v-card-subtitle
                        >Time: {{ inv.date_time | TimeFormat }}</v-card-subtitle
                    >
                    <v-card-text>
                        <v-chip-group active-class="primary--text" column>
                            <v-chip
    ...
    methods: {
        confirm(inv) {
            if (!this.roles.includes("user")) {
                this.alert = true;
                this.alertMessage = `${this.user.name} Doesn't has permission to remove invs.`;
