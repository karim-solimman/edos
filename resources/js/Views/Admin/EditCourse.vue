        ...
        <v-row v-if="!loading">
            <v-col>
                ...
                <v-chip
                    class="text-overline"
                    outlined
                    v-if="course && course.invs.length > 0"
                    ><v-icon left>mdi-calendar</v-icon
                    >{{ course.invs[0].date_time | DateFormat }}</v-chip
                >
                <v-chip
                    class="text-overline"
                    outlined
                    ...
                    >{{ course.invs[0].date_time | TimeFormat }}</v-chip
                >
                <v-chip
                    class="text-overline"
                    outlined
                    ...
                    >{{ course.invs[0].duration }} Hours</v-chip
                >
                <v-chip class="text-overline" outlined v-else>No invs</v-chip>
                <v-chip class="text-overline my-1" outlined
                    ...
                    / {{ course.credit_hours }} credit hours</v-chip
                >
            </v-col>
        </v-row>
        <v-row v-if="!loading">
            <v-col cols="12" lg="4" md="4">
                    ...
                    <v-card-text>
                        <v-form>
                            ...
                            <v-select
                                ...
                                item-text="name"
                                :value="
                                    course.department ? course.department.id : 1
                                "
                            ></v-select>
                            <v-btn
                                @click="updateInfo"
                                ...
                                block
                                text
                                ><v-icon left>mdi-plus</v-icon>Update</v-btn
                            >
                        </v-form>
                    </v-card-text>
                </v-card>
                ...
                <v-card
                    ...
                    v-if="!loading && course.invs.length > 0"
                >
                    <v-card-title>
                        <h1 class="text-h5 font-weight-light">
                    ...
                    <v-card-actions>
                        <v-btn
                            :loading="btnLoading2"
                            @click="updateDateAndTime"
                            block
                            color="primary"
                            text
                            ><v-icon left>mdi-pencil</v-icon>Change date and
                            time</v-btn
                        >
                    </v-card-actions>
                </v-card>
            ...
            <v-col cols="12" lg="8" md="8">
                ...
                >
                    <v-card-title
                        ><h1 class="text-h5 font-weight-light">
                            Remove all course invs
                        </h1></v-card-title
                    >
                    <v-card-text
                        ><strong>Warning</strong>, by removing invs from
                        {{ course.code }}, all invs will be permentantly removed
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: "actions", value: "actions", width: 140 }
            ],
            ...
            courseCodeRules: [
                ...
                v =>
                    ...
                    "Course code must be between 4 and 10 characters"
            ],
            courseName: null,
            courseNameRules: [
        ...
        axios({
            ...
            .then(response => {
                ...
                if (this.course.invs.length > 0) {
                    this.date = this.$options.filters.DateOnly(
                        this.course.invs[0].date_time
                    );
                    this.time = this.$options.filters.TimeOnly(
                        this.course.invs[0].date_time
                    );
                }
            })
    ...
    methods: {
        confirmDeleteInv(inv) {
            ...
            this.dialogText =
                ...
                this.$options.filters.DateFormat(inv.date_time) +
                " ?";
        ...
        updateInfo() {
            ...
            axios({
                ...
                .then(response => {
                    ...
                    this.course.department = this.departments.filter(item => {
                        return item.id == parseInt(this.courseDepartmentId);
                    })[0];
                })
                .catch(error => {
                    this.btnLoading1 = false;
