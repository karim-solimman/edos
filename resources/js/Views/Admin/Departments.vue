<template>
    <v-container>
        ...
        <Confrimation @dialog-closed="dialog=false" :confirmationText="dialogText" :dialog="dialog" :dialogData="dialogData" :onDeleteFunction="dialogFunction"  />
        <v-row v-if="!loading">
           <v-text-field
                v-model="search"
            ...
            <v-col>
                <v-data-table :headers="headers" :items="departments" :search="search">
                    ...
                    <template v-slot:[`item.actions`]="{item}">
                        <v-btn color="info" style="text-decoration: none" icon small :to="{name: 'departmentProfile', params:{id: item.id}}"><v-icon small>mdi-folder-open-outline</v-icon></v-btn>
                        ...
                        <v-btn style="text-decoration: none" icon small color="error" @click="confirmDelete(item)"><v-icon small>mdi-delete</v-icon></v-btn>    
                    </template>
                </v-data-table>
            </v-col>
        </v-row>
...
import Confrimation from '../../components/Confirmation.vue'
export default {
    components: {
        Loading, Alert, Confrimation
    },
    data(){
        return{
            loading: true,
