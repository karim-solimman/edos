        ...
        <v-row v-if="!loading">
            <v-col cols="12" md="6" lg="6">
                <v-card color="grey lighten-5">
                    <v-card-title class="text-h5 font-weight-light"
                        >Update Information</v-card-title
                    >
                ...
                <v-card color="grey lighten-5 mt-4">
                    <v-card-title class="text-h5 font-weight-light"
                        >Password Reset</v-card-title
                    >
                    <v-card-text>
                        <p class="text-body-1">
                            <strong color="error">Warning</strong>, by reseting
                            the user password, user won't be able to login again
                            until new registeration is done
                            <strong
                                >, user data and invs won't be removed</strong
                            >
                        </p>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn
                            ...
                            block
                            text
                            ><v-icon left>mdi-lock-reset</v-icon>reset</v-btn
                        >
                    </v-card-actions>
                </v-card>
                <v-card color="grey lighten-5 mt-4">
                    <v-card-title class="text-h5 font-weight-light"
                        >Remove all users invs</v-card-title
                    >
                    <v-card-text>
                        <p class="text-body-1">
                            <strong color="error">Warning</strong>, This action
                            can't be undo.
                            <strong
                                >, User name, department, email and password
                                will still the same!</strong
                            >
                        </p>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn
                            @click="confirmRemoveAllInvs"
                            color="error"
                            block
                            text
                            ><v-icon left>mdi-delete-forever</v-icon>remove all
                            invs</v-btn
                        >
                    </v-card-actions>
                </v-card>
            ...
            <v-col>
                <v-card class="mb-5" color="grey lighten-5">
                    ...
                    <v-card-actions>
                        <v-btn
                            :loading="depBtnLoading"
                            @click="updateDepartment"
                            color="primary"
                            block
                            text
                            ><v-icon left>mdi-pencil</v-icon>change</v-btn
                        >
                    </v-card-actions>
                </v-card>
                <v-card class="mb-5" color="grey lighten-5">
                    ...
                    <v-card-text>
                        <p class="text-body-1">
                            <strong>Warning</strong>, deleting user will detach
                            all invs from this user before deleting. Deleting
                            user will remove all related data to the user and
                            <strong>this action can't be undo.</strong>
                        </p>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn @click="confirmDeleteUser" block color="error"
                ...
                <v-card color="grey lighten-5">
                    ...
                    <v-card-text>
                        <v-data-table
                            v-if="user.invs && user.invs.length > 0"
                            :headers="headers"
                            :items="user.invs"
                        >
...
export default {
    ...
    data() {
        return {
            ...
            depBtnLoading: false,
            errorMessages: [{ name: "" }, { email: "" }, { department: "" }],
            countDown: 2
        };
    },
    mounted() {
