        ...
        <v-row v-if="!loading">
            <v-col>
                <h5 class="text-h5 font-weight-light">
                    {{ department.name }} <v-chip x-small>Edit</v-chip>
                </h5>
            </v-col>
        </v-row>
        <v-row v-if="!loading">
            <v-col cols="12" lg="4" md="4">
                ...
                >
                    ...
                    <v-spacer></v-spacer>
                    <v-card-actions>
                        <v-btn @click="updateName" block text color="info"
                            ><v-icon left>mdi-pencil</v-icon>update name</v-btn
                        >
                    </v-card-actions>
                </v-card>
            </v-col>
            <v-col cols="12" lg="4" md="4">
                ...
                >
                    ...
                    >
                    <v-card-text>
                        <strong>Warning</strong>, By flushing all the invs you
                        are detach all users from those invs and this action
                        can't be undo.
                    </v-card-text>
                    <v-spacer></v-spacer>
                    <v-card-actions>
                        <v-btn
                            @click="confirmFlushInvs(department)"
                            color="error"
                            block
                            ><v-icon left>mdi-close</v-icon>Flsuh all
                            invs</v-btn
                        >
                    </v-card-actions>
                </v-card>
            ...
            <v-col>
                ...
                >
                    ...
                    >
                    <v-card-text
                        ><strong>Warning</strong>, deleting / removing
                        {{ department.name }} department will remove all the
                        invs of this department also will unlink all courses
                        attached to the department and this can't be
                        undo.</v-card-text
                    >
                    <v-spacer></v-spacer>
                    <v-card-actions
            ...
            <v-col>
                <v-data-table :headers="headers" :items="department.courses">
                    ...
                    <template v-slot:[`item.date`]="{ item }">
                        <span v-if="item.invs.length > 0">{{
                            item.invs[0].date_time | DateFormat
                        }}</span>
                        <v-chip color="error" x-small v-else
                            ><v-icon x-small left
                    ...
                    <template v-slot:[`item.time`]="{ item }">
                        <span v-if="item.invs.length > 0">{{
                            item.invs[0].date_time | TimeFormat
                        }}</span>
                        <v-chip color="error" x-small v-else
                            ><v-icon x-small left
                                >mdi-information-outline</v-icon
                    ...
                    <template v-slot:[`item.rooms`]="{ item }">
                        <v-chip-group v-if="item.invs.length > 0">
                            <v-chip
                                small
                                v-for="inv in item.invs"
                                :key="inv.id"
                                >{{ inv.room.number }}</v-chip
                            >
                        </v-chip-group>
                        <v-chip color="error" x-small v-else
                            ><v-icon x-small left
        ...
        deleteCourse(courseId) {
            axios({
                ...
                .then(response => {
                    ...
                    this.alertType = "success";
                    this.alertMessage = response.data.message;
                    this.department.courses = this.department.courses.filter(
                        item => {
