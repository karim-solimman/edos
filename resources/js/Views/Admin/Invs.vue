<template>
    <v-container>
        ...
        <UserDialog
            :dialog="userDialog"
            :inv="inv"
            @user-dialog-closed="userDialog = false"
        />
        <v-row align="center" justify="center" v-if="!loading">
            ...
            <v-col
                class="my-auto mt-5 d-flex flex-row justify-space-around"
                cols="6"
                lg="3"
                md="3"
            >
        ...
        <v-row v-if="!loading && !toggleUsers && !toggleGroup">
            <v-col>
                <v-data-table
                    @current-items="exportData"
                    :headers="headers"
                    :items="filteredInvs"
                    :search="search"
                    sort-by="date_time"
                >
                    ...
                    <template v-slot:[`item.department`]="{ item }">
                        <span v-if="item.course.department">{{
                    ...
                    <template v-slot:[`item.actions`]="{ item }">
                        <v-btn
                            ...
                            icon
                            small
                            ><v-icon small @click="showInvUsers(item)"
                        ...
                        <v-btn
                            ...
                            ><v-icon small>mdi-calendar-outline</v-icon></v-btn
                        >
                        <v-btn
                            style="text-decoration: none"
                            class="ml-1"
                            icon
                            small
                            :to="{ name: 'edit-inv', params: { id: item.id } }"
        ...
        <v-row v-if="!loading && toggleGroup">
            <v-col>
                <v-row v-for="(inv, date) in filteredInvsGroups" :key="inv.id">
    ...
    methods: {
        getInvsGroups() {
            axios({
                ...
                .then(response => {
                    this.invsGroups = response.data.invs;
                })
                .catch(error => {
                    this.alert = true;
        ...
        confirm(inv) {
            ...
            this.dialogText = `Are you sure you want to delete inv on\n${this.$options.filters.DateFormat(
                inv.date_time
            )} at ${this.$options.filters.TimeFormat(inv.date_time)} for ${
                inv.course.code
            } - ${inv.course.name}`;
        },
        removeInv(invId) {
